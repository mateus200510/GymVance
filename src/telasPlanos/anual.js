...
function PlanoCard({ titulo, preco, precoAntigo, desconto, legenda, selecionado, destaque, onPress }) {
  return (
    <TouchableOpacity
      style={[
        styles.planoCard,
        selecionado && styles.planoCardSelecionado,
        destaque && selecionado && styles.planoCardDestaque,
      ]}
      onPress={onPress}
      activeOpacity={0.8}
    >
      {destaque && (
        <View style={styles.estrelaWrapper}>
          <Ionicons name="star" size={14} color="#FFD84D" />
        </View>
      )}
